...
const Packages: React.FC = () => {
  ...
  const packageStatusLabels: Record<string, { color: string; label: string }> = {
    ...
    IN_PROGRESS: { color: 'blue', label: t.package?.statuses?.GENERATING || 'Oluşturuluyor' },
    COMPLETE: { color: 'green', label: t.package?.statuses?.READY || 'Hazır' },
    SUBMITTED: { color: 'purple', label: t.package?.statuses?.DELIVERED || 'Teslim Edildi' },
    ACCEPTED: { color: 'cyan', label: t.material?.controlResults?.ACCEPTED || 'Kabul' },
  };
  ...
  const handleCreatePackage = async () => {
    try {
      ...
      await apiClient.post('/packages', {
        projectId, name: values.name, number: values.number,
  ...
  const columns = [
    ...
    { title: t.package?.period || 'Dönem', key: 'period', width: 200, render: (_: unknown, record: Package) => {
      if (record.periodStart && record.periodEnd) return `${dayjs(record.periodStart).format('DD.MM.YYYY')} — ${dayjs(record.periodEnd).format('DD.MM.YYYY')}`;
      if (record.periodStart) return dayjs(record.periodStart).format('DD.MM.YYYY');
      return t.app.noData;
    }},
    { title: t.doc?.title || 'Doküman', key: 'docCount', width: 110, render: (_: unknown, record: Package) => record.documentIds?.length || 0 },
  ...
  return (
    <div>
      ...
      <Modal title={t.package?.create} open={wizardVisible}
        ...
        {wizardStep === 0 && (
          <Form form={wizardForm} layout="vertical" style={{ maxWidth: 500 }}>
            ...
            <Form.Item name="number" label={t.doc?.number || 'No'}><Input /></Form.Item>
            <Row gutter={16}>
                ...
                <List dataSource={availableDocs} locale={{ emptyText: t.app.noData }}
                  renderItem={(doc: Document) => (
                    <List.Item><Checkbox value={doc.id} style={{ width: '100%' }}>
                      <Space><FileTextOutlined /><Text strong>{doc.number}</Text><Text>{doc.title}</Text><Tag color="green">{t.doc?.statuses?.SIGNED}</Tag><Text type="secondary">{dayjs(doc.createdAt).format('DD.MM.YYYY')}</Text></Space>
                    </Checkbox></List.Item>
                  )} />
              </Checkbox.Group>
            </Space>
        ...
        {wizardStep === 2 && (
          <div>
            <Descriptions bordered column={1} size="small" style={{ marginBottom: 16 }}>
              ...
              <Descriptions.Item label={t.doc?.number || 'No'}>{wizardForm.getFieldValue('number') || '—'}</Descriptions.Item>
            ...
            {selectedDocIds.length > 0 && (
              <Card title={t.package?.includedDocs} size="small">
                <List dataSource={availableDocs.filter((d) => selectedDocIds.includes(d.id))} size="small"
                  renderItem={(doc: Document, index: number) => <List.Item><Text>{index + 1}. {doc.number} — {doc.title}</Text></List.Item>} />
              </Card>
            )}
          </div>
        )}
      ...
      <Drawer title={selectedPackage ? `${selectedPackage.number || ''} ${selectedPackage.name}` : t.package?.title} open={detailDrawerVisible}
          ...
          {selectedPackage && (
            <>
              <Descriptions bordered column={2} size="small" style={{ marginBottom: 24 }}>
                ...
                <Descriptions.Item label={t.doc?.number || 'No'}>{selectedPackage.number || '—'}</Descriptions.Item>
                ...
                <Descriptions.Item label={t.package?.period}>{selectedPackage.periodStart ? `${dayjs(selectedPackage.periodStart).format('DD.MM.YYYY')} — ${selectedPackage.periodEnd ? dayjs(selectedPackage.periodEnd).format('DD.MM.YYYY') : '...'}` : t.app.noData}</Descriptions.Item>
                <Descriptions.Item label={t.doc?.title}>{selectedPackage.documentIds?.length || 0}</Descriptions.Item>
              ...
              {selectedPackage.documents && selectedPackage.documents.length > 0 ? (
                <List dataSource={selectedPackage.documents}
                  renderItem={(doc: Document, index: number) => (
                    <List.Item>
                      <List.Item.Meta avatar={<FileTextOutlined style={{ fontSize: 20 }} />}
                        ...
                        description={<Space><Tag>{doc.type}</Tag><Tag color="green">{t.doc?.statuses?.SIGNED}</Tag><Text type="secondary">{dayjs(doc.createdAt).format('DD.MM.YYYY')}</Text></Space>} />
                    </List.Item>
                  )} />
              ) : (
